...
async def check_bedwars_requirements(player_data):
    ...
    star = player_data["player"]["achievements"]["bedwars_level"]
...
async def check_murdermystery_requirements(player_data):
    ...
    if wins > settings.MURDERMYSTERY_WINS:
        return True
...
async def check_if_meets_requirements(username):
    ...
    if name == None:
        ...
        name, uuid = await get_mojang_profile(username)
    player_data = await get_hypixel_player(uuid)
    ...
    meets_network_level_requirements = await check_network_level_requirements(player_data)
    ...
    if any([meets_skywars_requirements, meets_bedwars_requirements, meets_duels_requirements, meets_arcade_requirements,
            meets_murderymystery_requirements, meets_network_level_requirements, meets_gexp_requirements]) and not meets_max_guild_level:
        meets_requirements = True

    return guildless, meets_requirements
